...
/**
 * Fluxo Three (Credcesta)
 */
trait TraitFluxoThree
{
    public static function fluxo($message, $phone, $step_id, $id_lead)
    {
        ...
        if ($message == '*') {
            ...
            Lead::saveStepsLead($user['user_telefone'], $user['id'], 4, MSG_2, 1, $user['fluxo_id']);
        ...
        if ($message == 'Quero confirmar minha proposta') {
            Lead::saveStepsLead($user['user_telefone'], $user['id'], 3, MSG_FLUXO_114, 1, $user['fluxo_id']);
        ...
        switch ($user['step_whatsapp']) {
                ...
                if (Tools::verifyCPF($message) == 1) {
                    if ($user['user_cpf'] == Tools::limpaCPF($message)) {
                        ...
                        StepsWhats::insert([
                            ...
                            'lead_id' => $user['id'],
                            'id_fluxo' => $user['fluxo_id'],
                            'step' => 2,
                            'channel' => 'whatsapp'
                        ]);

                        Lead::saveStepsLead($user['user_telefone'], $user['id'], 5, MSG_FLUXO_101, 2, $user['fluxo_id']);
                        ...
                        if ($user['tentativas_whatsapp'] < 3) {
                            ...
                            switch ($user['tentativas_whatsapp']) {
                                case 1:
                                    Lead::saveStepsLead($user['user_telefone'], $user['id'], 6, $msg, 1, $user['fluxo_id']);
                                    break;
                                case 2:
                                    Lead::saveStepsLead($user['user_telefone'], $user['id'], 7, $msg, 1, $user['fluxo_id']);
                                    break;
                            }
                            ...
                            //$phone_user, $lead_id, $step_id, $step_whats_message, $step_id_whats (constants)
                            Lead::saveStepsLead($user['user_telefone'], $user['id'], 18, MSG_FLUXO_106, 1, $user['fluxo_id']);
                ...
                switch (Tools::formatDecision($message)) {
                    case 'Sim':
                        if (isset($user['seguro_prestamista_confirma']) && !empty($user['seguro_prestamista_resumo'])) {
                            ...
                            //$phone_user, $lead_id, $step_id, $step_whats_message, $step_id_whats (constants)
                            Lead::saveStepsLead($user['user_telefone'], $user['id'], 8, $prestamista_confirma, 3, $user['fluxo_id']);
                            ...
                            //$phone_user, $lead_id, $step_id, $step_whats_message, $step_id_whats (constants)
                            Lead::saveStepsLead($user['user_telefone'], $user['id'], 8, $msg, 4, $user['fluxo_id']);
                    ...
                    case 'Nao':
                        ...
                        //$phone_user, $lead_id, $step_id, $step_whats_message, $step_id_whats (constants)
                        Lead::saveStepsLead($user['user_telefone'], $user['id'], 19, MSG_FLUXO_103, 4, $user['fluxo_id']);
            ...
            case '3':
                ...
                if ($msg == 'Sim' || $msg == 'Nao') {
                    ...
                    //$phone_user, $lead_id, $step_id, $step_whats_message, $step_id_whats (constants)
                    Lead::saveStepsLead($user['user_telefone'], $user['id'], $step_id, $msg, 4, $user['fluxo_id']);
